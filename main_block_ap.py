...
def evaluate(model, tokenizer, args, logger):
    ...
    block_class_name = get_transformer_layers(model)[0].__class__.__name__
...
def main():
    ...
    if args.resume_quant:
        ...
        logger.info(f"resolved model kind: {resolve_model_kind(model, requested=args.model_kind)}")
        ...
        resolved_model_kind = resolve_model_kind(config, requested=args.model_kind)
        logger.info(f"resolved model kind: {resolved_model_kind}")
        ...
        model = load_auto_model_for_causal_lm(
            ...
            config=config,
            device_map='cpu',
            torch_dtype=resolve_model_dtype(config),
            trust_remote_code=args.trust_remote_code,
            token=hf_token,
        )
