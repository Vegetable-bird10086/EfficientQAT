...
import os
import utils
from hf_compat import build_trainer_processing_kwargs
from hf_compat import load_auto_config
from hf_compat import resolve_model_kind
from hf_compat import resolve_hf_token
from hf_compat import resolve_model_dtype
...
class ModelArguments:
    ...
    model_kind: Optional[str] = field(
        default="auto",
        metadata={"help": "Model family routing hint. Choices: auto, llama, qwen3."}
    )
...
class GenerationArguments:
    ...
    generation_use_cache: Optional[bool] = field(
        default=True,
        metadata={"help": "Whether to enable KV-cache during generation config construction."}
    )
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    quant_config = load_auto_config(
        args.quant_model_path,
        trust_remote_code=args.trust_remote_code,
        token=hf_token,
    )
    args.model_kind = resolve_model_kind(quant_config, requested=args.model_kind)
    ...
    model_dtype = resolve_model_dtype(model)
...
def train():
    ...
    generation_kwargs = vars(generation_args).copy()
    generation_kwargs["use_cache"] = generation_kwargs.pop("generation_use_cache")
    training_args.generation_config = transformers.GenerationConfig(**generation_kwargs)
    ...
    trainer = Seq2SeqTrainer(
        ...
        **build_trainer_processing_kwargs(Seq2SeqTrainer, tokenizer),
    ...
    if args.do_ppl_eval:
        class PPLvalCallback(transformers.TrainerCallback):
            ...
            def on_evaluate(self, args=None, state=None, control=None, model=None, **kwargs):
                results = test_ppl(trainer.model, tokenizer, datasets=['wikitext2','c4'],ppl_seqlen=2048)
                logger.info(results)
                trainer.log(results)
