...
import pdb
import gc
from hf_compat import get_final_norm_module, get_input_embedding_module, get_output_head_module, get_rotary_embedding_module, get_transformer_layers
...
def block_ap(
    ...
    embed_tokens = get_input_embedding_module(model).to(dev)
    model_norm = get_final_norm_module(model).to(dev)
    model.model.embed_tokens = embed_tokens
    model.model.norm = model_norm
    rotary_emb = get_rotary_embedding_module(model)
    ...
    for block_index in range(len(layers)):
        ...
        block_rotary_emb = get_rotary_embedding_module(model)
    ...
    if isinstance(output_head, torch.nn.Linear):
        has_lm_head_override = quant_config is not None and any(rule.matches("lm_head") for rule in quant_config.overrides)
        if has_lm_head_override:
            output_spec = quant_config.resolve("lm_head", output_head.in_features)
            logger.info("=== Quantize output head lm_head ===")
            fake_output = int_linear_fake.QuantLinear(
                output_head.cpu(),
                output_spec.bits,
                output_spec.resolved_group_size(output_head.in_features),
                quant_spec=output_spec,
            )
            if args.real_quant:
                scales = fake_output.weight_quantizer.scale.clamp(1e-4, 1e4).detach()
                zeros = fake_output.weight_quantizer.zero_point.detach().round()
                dim0 = fake_output.weight.shape[0]
                scales = scales.view(dim0, -1).transpose(0, 1).contiguous()
                zeros = zeros.view(dim0, -1).transpose(0, 1).contiguous()
                q_linear = int_linear_real.QuantLinear(
                    fake_output.quant_spec.bits,
                    fake_output.weight_quantizer.group_size,
                    fake_output.in_features,
                    fake_output.out_features,
                    fake_output.bias is not None,
                    mapping=fake_output.quant_spec.mapping,
                    train_scale=fake_output.quant_spec.train_scale,
                    train_zero_point=fake_output.quant_spec.train_zero_point,
                    quant_spec=fake_output.quant_spec,
                )
                q_linear.pack(fake_output.cpu(), scales.float().cpu(), zeros.float().cpu())
                model.lm_head = q_linear
            else:
                fake_output.half()
                quant_inplace(fake_output)
                fake_output.set_quant_state(weight_quant=False)
                model.lm_head = fake_output.cpu()
            logger.info("pack quantized lm_head finished")
