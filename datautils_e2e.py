...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            train_files = sorted(str(path) for path in parquet_dir.glob("train-*.parquet")) if parquet_dir.exists() else []
            validation_files = sorted(str(path) for path in parquet_dir.glob("validation-*.parquet")) if parquet_dir.exists() else []
            if train_files:
                data_files = {"train": train_files}
                if validation_files:
                    data_files["validation"] = validation_files
                print(f"load local redpajama parquet from {parquet_dir}")
    ...
    if args.dataset in ['c4', 'redpajama']:
        ...
        if args.max_train_samples is not None and len(train_raw_data) > args.max_train_samples:
            train_raw_data = train_raw_data[: args.max_train_samples]
        if args.max_eval_samples is not None and len(eval_raw_data) > args.max_eval_samples:
            eval_raw_data = eval_raw_data[: args.max_eval_samples]
